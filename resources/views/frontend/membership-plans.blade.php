...
@section('content')
            ...
            <div class="inr-bnr-text">
                ...
                <nav>
                    <ol class="cd-breadcrumb custom-separator">
                        <li><a href="">Home</a></li>
                        <li class="current"><em>Membership</em></li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    @foreach ($plans as $key => $plan)
                        <div class="col-xl-3 col-lg-6 col-12">
                            <div class="mem-box-wrap">
                                ...
                                <div class="price-box">
                                    <h2><span>USD</span>{{ $plan['plan_price'] == 0 ? 'Free' : '$' . $plan['plan_price'] }}<span>({{ $plan['plan_type'] }})</span>
                                    </h2>
                                </div>
                                <div class="mem-list">
                                    <ul>
    ...
    <section class="mem-sec">
        <div class="container">
            <div class="mem-sec-wrap">
                <div class="row justify-content-center align-items-center">
                    @if (count($video_call_prices) > 0)
                        @foreach ($video_call_prices as $key => $video_call_price)
                            <div class="col-xl-3 col-lg-6 col-12">
                                <div class="telehealth_plan">
                                    <span>{{ $video_call_price['title'] }}</span>
                                    <h5>{{ $video_call_price['price'] > 0 ? '$' . $video_call_price['price'] : 'Free' }} / {{ $video_call_price['duration'] ? $video_call_price['duration'] . ' Min' : 'Free' }}</h5>
                                </div>
                            </div>
                        @endforeach
                    @else
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == true) {
                            ...
                            $('.membership-price').text(response.data.plan_price == 0 ? 'Free' :
                                response.data.plan_price);
